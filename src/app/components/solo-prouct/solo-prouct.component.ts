...
export class SoloProuctComponent implements OnInit {
  ...
  constructor(private toaster:ToastrService,private router: Router, private myActivatedRoute: ActivatedRoute, private myService: ProductsService, 
  ...
  UpdateProduct() {
    ...
    this.subscriber = this.myService.UpdateProduct(this.product._id, formData)
      .subscribe((res: string) => {
        if (res) {
        this.toaster.success("Product Updated Succesfuly")
          this.product = res;
        }
      },
        (err) => {
  ...
  deletproduct(id) {
    ////////////////////
       this.myService.DeletFromCart(this.user._id, this.id)
        .subscribe((prod) => {
          // this.Products = this.Products.filter(p => p.product._id != id);
          this.communicationService.emitChange(this.user.products.length);
        }, (err) => {
        });
    //////////////////
